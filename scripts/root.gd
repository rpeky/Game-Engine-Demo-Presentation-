...
func try_interact_2d():
	...
	# Pick two atlas coords to toggle between (replace with real coords)
	var A := Vector2i(15, 3)
	var B := Vector2i(16, 3)

	# If empty, you must choose a valid source_id (usually 0)
	if cur_source == -1:
		cur_source = 0
		cur_atlas = A
		cur_alt = 0
	...
	layer.set_cell(cell, cur_source, new_atlas, 0)

	# Optional: still call your interact script
